    ...
    <header className="bg-formBG text-white sticky top-0 z-50 py-10" id="navigation-bar">
      ...
      <NavLink to="/">
        ...
        <nav>
          ...
          <div onClick={openModal ? '' : showSidebar} className="block mlg:hidden">
            <img src={menu} alt="Menu" className="w-8 h-6 cursor-pointer" />
          </div>
          <AnimatePresence>
            {toggleSidebar && (
              <motion.ul
                className="fixed top-0 text-center text-[18px] xxs:text-[25px] xxsm:text-[30px] left-0 flex flex-col items-start justify-start bg-primary sidebar w-full"
                initial={{ x: "-100%" }}
                animate={{ x: 0 }}
                exit={{ x: "-100%" }}
              ...
              >
                ...
                <li className="to-the-center p-0 my-4" onClick={hideSidebar}>
                  <a href="#"></a>
                  <button
